...
## Last Updated: 2026-04-27 (Session 66 — Restore desktop Google auth button fix)
...
## What Was Done This Session (2026-04-27 — Restore desktop Google auth button fix)

### Auth Google button
- Checked the previous fix documented in Session 26/32: the reliable desktop Google Identity Services path is an offscreen but full-size `340px x 44px` `<GoogleLogin>` button, triggered by the custom themed button through a ref.
- **`frontend/app/(auth)/login/page.tsx`** — restored that documented offscreen full-size ref-click pattern and removed the temporary transparent overlay / measured-width iframe approach.
- **`frontend/app/(auth)/register/page.tsx`** — restored the same pattern so login and register use matching Google auth plumbing.
- Kept the existing Google credential exchange, `storeSession()` CSRF retry, mandatory set-password modal flow, and missing-client-id disabled guard unchanged.
