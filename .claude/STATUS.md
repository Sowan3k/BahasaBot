...
## Last Updated: 2026-06-09 (Session 83 — Page loading performance + backend N+1 fix)
...
| Notification System (Phase 17) | ✅ Complete + Relocated (Session 18) + Clear-all | GET /api/notifications/ (last 20 + unread_count), POST mark-read, POST read-all, DELETE /api/notifications/ (clear-all); NotificationBell (60s polling, unread badge) + NotificationPanel; bell moved from floating fixed layout.tsx overlay into AppSidebar (mobile header + desktop sidebar footer) with panelSide/panelDirection props |
...
| Frontend Performance Optimizations | ✅ Complete + Session 83 | Session cache in api.ts (60s TTL, eliminates /api/auth/session round-trip on every API call); profile fetch deduplication via shared useQuery(['profile']) key in AppSidebar + OnboardingChecker; login router.refresh() race condition removed; redirect loading overlay added. **Session 83**: Dashboard vocab/grammar/quiz-history tabs converted from manual useEffect+fetch to useQuery — repeat tab switches instant (cache hit, background refetch); handleVocabDelete now invalidates React Query cache instead of manual refetch; Settings/Profile page replaced manual fetch with shared ["profile"] useQuery — page loads instantly when sidebar has already fetched; Journey page: removed 4 redundant useEffect state syncs + 6 redundant local state vars — derived directly from query data (roadmap, fetchError, userName, userEmail, provider, history); Games page: added onMouseEnter prefetch so game JS chunks pre-download before the user clicks Play; Chat History backend list_sessions N+1 fixed — 20 sessions × 3 queries → 3 bulk aggregation queries (60 DB queries → 3). |
...
### Problem
Logged-in users who navigated to `/pricing` (e.g. from the sidebar) and clicked the floating "Back" pill were sent to `/login`. The login page calls `sessionStorage.removeItem()` on `chatbot_messages`, `chatbot_session_id`, and `tip_dismissed` — wiping client-side state and appearing as a logout even though the NextAuth session itself was intact.

The same page is publicly accessible from the auth pages, so the back button needed to be context-aware rather than always pointing at `/login`.

### Root Cause
`frontend/app/pricing/page.tsx` — back button hardcoded `href="/login"`:
```tsx
<Link href="/login" ...>  ← always sent authenticated users to login page
```

### Fix
`frontend/app/pricing/page.tsx` — 3 changes:
- Imported `useSession` from `next-auth/react`
- Added `const { status } = useSession()` + `const backHref = status === "authenticated" ? "/dashboard" : "/login"`
- Back button now uses `href={backHref}` — resolves to `/dashboard` when logged in, `/login` when not
- Bottom "Already have an account? Sign in" row now renders "Go to Dashboard" link when `status === "authenticated"` (avoids showing a sign-in prompt to a user who is already signed in)

### Verified
- `tsc --noEmit`: 0 errors
- Unauthenticated visitor: Back → `/login` ✓
- Authenticated user: Back → `/dashboard` ✓
...
## What Was Done This Session (2026-05-09 Session 71 — Glassy pricing UI redesign)

### Goals
Redesign Phase 25 subscription/pricing pages with an animated glassy aesthetic using WebGL and glass-morphism cards.

### New files created
- `frontend/components/ui/multi-type-ripple-buttons.tsx` — `RippleButton` component; supports default/hover/hoverborder/ghost variants; JS click-ripple animation via keyframes; default ripple color uses `--button-ripple-color` CSS var; hoverBaseColor defaulted to BahasaBot olive `#8d9d4f`
- `frontend/components/ui/animated-glassy-pricing.tsx` — exports `ShaderCanvas` (WebGL animated ring shader; background RGB adapted: light = cream `#e4d7b0`, dark = near-black `#111110`; foreground circle colors shifted to warm olive/wheat tones); `GlassCard` (generic `backdrop-blur-[14px]` glass wrapper with `featured` ring variant); standalone `PricingCard` (flat-props API) and `ModernPricingPage` for future use

### Modified files
- `frontend/components/subscription/PricingCard.tsx` — dropped GlowCard; now uses `GlassCard` + `RippleButton`; large extralight plan name heading; price in `font-heading font-extralight`; gradient divider; `text-primary` Check icons; styled for featured (ring + scale) vs standard cards
- `frontend/app/pricing/page.tsx` — full visual redesign: `BackgroundPaths` → `ShaderCanvas` (fixed WebGL background); sticky nav uses `backdrop-blur-[20px] bg-background/40`; hero title uses `bg-gradient-to-r from-foreground via-primary to-accent` clip-text gradient; comparison table wrapped in `GlassCard`; FAQ accordion items wrapped in `GlassCard` (glass hover states); 7-day trial callout and footer CTA use `GlassCard featured`; all CTAs replaced with `RippleButton`; all section headings use `font-light tracking-tight`
- `frontend/app/(dashboard)/settings/billing/page.tsx` — keeps `BackgroundPaths` (inside dashboard layout); all cards upgraded to `GlassCard`; buttons upgraded to `RippleButton`; gradient divider separating plan name from feature list
- `frontend/app/globals.css` — added `--button-ripple-color: rgba(0,0,0,0.12)` (light) and `rgba(255,255,255,0.18)` (dark) CSS variables for RippleButton
